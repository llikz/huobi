...
//      mWindow.setHeight(ViewGroup.LayoutParams.WRAP_CONTENT);
  ...
  private void initControllerView(View v) {
    ...
    mFullButton=(ImageButton) v.findViewById(getResources().getIdentifier("mediacontroller_play_full", "id", mContext.getPackageName()));

    if (mFullButton != null) {
      mFullButton.requestFocus();
      mFullButton.setOnClickListener(new OnClickListener() {
        @Override
        public void onClick(View v) {
           if(mFullListener!=null){
             mFullListener.onClick(v);
           }
        }
      });
    }
  ...
  public void setFullListener(OnClickListener l){
     mFullListener = l;
  }
...
//        mWindow.setAnimationStyle(mAnimStyle);
        setWindowLayoutType();
        windHeight= location[1];
       int  wheight=mAnchor.getMeasuredHeight();
//        mWindow.showAtLocation(mAnchor, Gravity.NO_GRAVITY, anchorRect.left, 0);

//        mWindow.showAtLocation(mAnchor,Gravity.NO_GRAVITY,anchorRect.left,wheight-windHeight);
//        mWindow.showAtLocation(mAnchor, Gravity.BOTTOM, anchorRect.left, anchorRect.bottom);
        ...
        if(mAnchor instanceof RelativeLayout){
            ...
            ((RelativeLayout) mAnchor).removeView(mRoot);
            ((RelativeLayout) mAnchor).addView(mRoot,lp);
        }
       if(mRoot!=null)mRoot.setVisibility(VISIBLE);
      }
      mShowing = true;
  ...
  public void fillFullWindow(View parent){
    mShowing = false;
    if(parent!=null){
//      ((RelativeLayout)mAnchor).removeAllViews();
//      TextView titleTv=new TextView(getContext());
//      titleTv.setText("hello");
//      RelativeLayout.LayoutParams lp=new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,RelativeLayout.LayoutParams.WRAP_CONTENT);
//      lp.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
//      titleTv.setLayoutParams(lp);
//
//      ((RelativeLayout)mAnchor).addView(titleTv);
     // mWindow.showAtLocation(parent,Gravity.TOP,20,20);
    }
  }
...
////          mWindow.dismiss();
  ...
  boolean isBarShow= true;
  public void setNavShow(boolean show){
    this.isBarShow = show;
  }
