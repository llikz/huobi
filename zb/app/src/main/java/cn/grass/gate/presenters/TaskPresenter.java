...
public class TaskPresenter {
    ...
    public TaskPresenter(TaskView v){
        view = v;
    }
    public int TOKEN_GET_TASK_LIST=0x01;
    public void getTaskList(int pageIndex,int pageSize){
        HashMap<String,String> map=new HashMap<>();
        map.put("pageIndex",""+pageIndex);
        map.put("pageSize",""+pageSize);
        HttpManager.getInstance().getRequestForGson(GrassApp.getInstance(), Api.URL_GET_TASK_LIST,
                map, new RequestCallBack() {
                    @Override
                    public void onSuccess(int callId, String content, String msg) {
                        WaitTask bean = GsonImpl.get().toObject(content,WaitTask.class);
                        if(view!=null){
                            view.taskList(bean);
                            return;
                        }
    ...
    public int TOKEN_GET_LIKE_TASK_LIST=0x02;
    public void getLikeTaskList(int pageIndex,int pageSize,String searchStr){
        HashMap<String,String> map=new HashMap<>();
        map.put("pageIndex",""+pageIndex);
        map.put("pageSize",""+pageSize);
        map.put("titleKey",searchStr);
        HttpManager.getInstance().getRequestForGson(GrassApp.getInstance(), Api.URL_GET_LIKE_TASK_LIST,
                map, new RequestCallBack() {
                    @Override
                    public void onSuccess(int callId, String content, String msg) {
                        WaitTask bean = GsonImpl.get().toObject(content,WaitTask.class);
