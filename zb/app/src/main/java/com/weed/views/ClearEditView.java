...
public class ClearEditView extends EditText implements OnFocusChangeListener,
        TextWatcher {
    /**
     * 删除按钮的引用
     */
    ...
    private void init() {
//        int pad=DisplayUtil.px2dip(getContext(),5);
//        setPadding(pad,pad,2*pad,pad);
        // 获取EditText的DrawableRight,假如没有设置我们就使用默认的图片
        mClearDrawable = getCompoundDrawables()[2];
        if (mClearDrawable == null) {
            mClearDrawable = getResources().getDrawable(
                    R.mipmap.common_bt_search_delete_normal);
        }
        mClearDrawable.setBounds(0, 0, mClearDrawable.getIntrinsicWidth(),
                mClearDrawable.getIntrinsicHeight());

        setHintTextColor(getResources().getColor(R.color.color_aaa));
    ...
    protected void setClearIconVisible(boolean visible) {
        ...
        rightDrawable=null;
        Drawable right = visible ? mClearDrawable : rightDrawable;
